...
void sphereCollisions(const grid_t *grid,
  ...
  double sphereBounceFactor = 1;
  double shapeBounceFactor = 1.8;
  ...
  for(int id=0;id<Nshapes;++id){
	...
	if(!(vdotdX1<0 && vdotdX2>0)){
	  ...
	  shape.sphere.newVelocity =
	    vectorAdd(shape.sphere.newVelocity, vectorScale(sphereBounceFactor*dVdotdX, dX));
	...
	if(dist<shape.sphere.radius){
	  ...
	  if(vdotdisp<0){
	    ...
	    shape.sphere.newVelocity =
	      vectorAdd(shape.sphere.newVelocity, vectorScale(-shapeBounceFactor*vdotn, n));
	  }
      ...
      //printf("%lf %lf %lf\n", shape.sphere.velocity.x,shape.sphere.velocity.y,shape.sphere.velocity.z);
